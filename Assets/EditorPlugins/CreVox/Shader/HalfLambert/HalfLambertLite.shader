...
//-------------------------------------------------------------------------------------
    ...
    SubShader {
        ...
        void surf (Input IN, inout SurfaceOutput o) {
            ...
            fixed4 sp = tex2D (_SpecGlossMap, IN.uv_MainTex);
            o.Specular = sp.rgb * _SpecColor;
...
//        }
        ...
        half4 LightingHalf (SurfaceOutput s, half3 viewDir, UnityGI gi) {
            ...
            //_WrapAmount = 1-_WrapAmount;
            half diff = saturate((NdotL + _WrapAmount) / ((1 + _WrapAmount) * (1 + _WrapAmount)));
            fixed3 diff_color = gi.light.color * diff * s.Albedo;

            half3 h = normalize (gi.light.dir + viewDir);
            float nh = max (0, dot (s.Normal, h));
            float spec = pow (nh, s.Gloss * 48 / (1 + _WrapAmount)) * s.Gloss * 8;
            fixed3 spec_color = s.Specular * gi.light.color * spec * (1 - _WrapAmount);
