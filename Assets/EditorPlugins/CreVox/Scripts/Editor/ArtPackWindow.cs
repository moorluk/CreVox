...
namespace CreVox
{
    ...
    public class ArtPackWindow : EditorWindow
    {
        ...
        void InitContent ()
        {
            ...
            //Log
            string logCell = "";
            foreach (var a in _artPacks) {
                foreach (var c in _categories)
                    logCell += a + "/" + c + ": " + _itemCells [a] [c].Count + "\n";
                logCell += " -------------\n";
            }
            Debug.Log (logCell);
                        ...
                        foreach (Dungeon d in vm.dungeons) {
                            foreach (BlockItem b in d.volumeData.blockItems) {
                                if (b.pieceName == oldName) {
                                    Debug.Log (d.volumeData.name + "(" + b.BlockPos + "): " + b.pieceName);
                                    b.pieceName = newName != "" ? newName : null;
                                }
                            }
                                ...
                                foreach (BlockAir b in c.blockAirs) {
                                    for (int i = 0; i < b.pieceNames.Length; i++) {
                                        if (b.pieceNames [i] == oldName) {
                                            log += d.volumeData.name + "(" + b.BlockPos + ")[" + i + "]: " + b.pieceNames [i] + "\n";
                                            b.pieceNames [i] = newName != "" ? newName : null;
                                        }
                                    }
                                }
                            }
        ...
        void UpdateAppDict ()
        {
            ...
            foreach (string apName in _artPacks) {
                if (apName == "LevelPieces")
                    continue;
                foreach (VGlobal.ArtPackParent app in vg.artPackParentList) {
                    if (app.pack == apName) {
        ...
        void UpdateAppList ()
        {
            ...
            foreach (KeyValuePair<string,string> k in _pDict) {
                var _a = new VGlobal.ArtPackParent ();
                _a.pack = k.Key;
